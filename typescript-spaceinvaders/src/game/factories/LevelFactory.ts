...
export class LevelFactory {
    ...
    public static getEmptyMap(numCols = 5, numRows = 3): number[][] {
        ...
        let ids = [Entity.ENEMY_1, Entity.ENEMY_2, Entity.ENEMY_3];
    ...
    public static generateLevel(levelModel: LevelModel, level: number): void {
        ...
        let map: number[][] = this.getLevelMapByLevel(level);
        let enemy: Entity;
        ...
        let cannon: Entity = EntityPool.getEntity(Entity.CANNON);
    ...
    private static getLevelMapByLevel(level: number): Array<Array<number>> {
        let levelData: LevelData = this._maps[level] || this._maps[0];
