...
export class TestSequenceWithHooksCommand extends SequenceMacro {
    public prepare(): void {
        this.add(ReportStringCommand).withPayloads("Command 1").withHooks(ReportHook);
        this.add(ReportStringCommand).withPayloads("Command 2").withHooks(ReportHook, ReportHook);
